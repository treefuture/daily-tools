...
def send_click_to_all(window_list, coords_list, screen_coords=None):
    """向多个窗口发送点击，每个窗口对应自己的坐标
    Args:
        window_list: 窗口句柄列表
        coords_list: 客户区坐标列表
        screen_coords: 屏幕绝对坐标 (用于 physical 模式)
    """
    if CLICK_METHOD == "physical" and screen_coords:
        # 物理鼠标点击使用屏幕绝对坐标
        import pyautogui
        x, y = screen_coords
        print(f"  🖱️ 物理点击 - 移动鼠标到: ({x}, {y})")
        pyautogui.moveTo(x, y, duration=0.1)  # 移动鼠标并显示轨迹
        print(f"  🖱️ 物理点击 - 执行点击动作")
        ...
        if pt3 is not None:
            ...
            if not img1_clicked and has_img1:
                ...
                if pt1 is not None:
                    ...
                    send_click_to_all(inner_windows, coords1, screen_coords=(abs_x1, abs_y1))
                    img1_clicked = True
                ...
                if pt2 is not None:
                    ...
                    send_click_to_all(inner_windows, coords2, screen_coords=(abs_x2, abs_y2))
                else:
                    print("  ⚠️ 未找到2.jpg，等待下次重试...")
